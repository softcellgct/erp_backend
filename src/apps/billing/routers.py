...
@router.post("/invoices", response_model=InvoiceResponse, tags=["Billing - Fee Heads"])
async def create_invoice(
    request: Request, payload: InvoiceCreate, db: AsyncSession = Depends(get_db_session)
):
    try:
        invoice = await billing_service.create_invoice(db, payload)
        return invoice
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_invoice(
    request: Request, invoice_id: str, db: AsyncSession = Depends(get_db_session)
):
    stmt = (
        select(Invoice)
        ...
        .options(
            ...
            joinedload(Invoice.status_history),
        )
    )
    result = await db.execute(stmt)
